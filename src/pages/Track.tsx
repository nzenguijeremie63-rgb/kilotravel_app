...
export default function Track() {
  ...
  // Realtime update when reservation status changes
  useEffect(() => {
    if (!reservation?.id) return;

    const channel = supabase
      .channel(`tracking-${reservation.id}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          setReservation((prev) => (prev ? { ...prev, ...(payload.new as any) } : null));
        }
      )
      .subscribe();
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-center gap-6 py-4 px-4 bg-muted rounded-lg">
                    <div className="text-center flex flex-col items-center">
                      <CountryFlag country={reservation.cargo_offers.departure_country} flag={reservation.cargo_offers.departure_country_flag} size="md" />
                      <p className="text-sm font-medium mt-1">{reservation.cargo_offers.departure_city}</p>
                    </div>
                    <Plane className="h-5 w-5 text-secondary rotate-45" />
                    <div className="text-center flex flex-col items-center">
                      <CountryFlag country={reservation.cargo_offers.arrival_country} flag={reservation.cargo_offers.arrival_country_flag} size="md" />
                      <p className="text-sm font-medium mt-1">{reservation.cargo_offers.arrival_city}</p>
                    </div>
                  </div>
